...
public class DateUtil {
    ...
    public static String getTodayYYMMDDHHMMSS() {
        return dateToString(new Date(), TIME_FORMAT_YY);
    }
...
//        return (nowDayNum - dayNum);
     ...
     * 判断某个日期是否跟另一个日期一样
     */
    public static int dateCompareDateTime(Date oneDate, Date twodate) {

        SimpleDateFormat sf = new SimpleDateFormat(DATETIME_FORMAT);
        return (stringToDate(sf.format(oneDate), DATETIME_FORMAT))
                .compareTo(stringToDate(sf.format(twodate), DATETIME_FORMAT));
    }
     ...
     * @return
     */
    public static int getDayOfWeek_ZH(Calendar cal ) {
     ...
     * @return
     */
    public static Date getYesterday(Calendar cal) {
    ...
    /**
     * 取上个月最后一天的时间 到 23：59：59
     *
     * @return
     */
    public static Date lastMonLastDay(Calendar cal) {
       
    	cal.set(Calendar.DAY_OF_MONTH, 1);//设置本月1日
        cal.add(Calendar.DAY_OF_MONTH, -1); //本月1日减一天
        cal.set(Calendar.HOUR_OF_DAY, 23);
		cal.set(Calendar.MINUTE, 59);
		cal.set(Calendar.SECOND, 59);
    ...
    public static void main(String[] args) {
    	...
    	System.out.println(DateUtil.dateToString(currentCal.getTime()));
    	System.out.println("week:"+DateUtil.getDayOfWeek_ZH(currentCal));
    } 
}
